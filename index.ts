...
const GEOCODING_API_KEY = 'owYaZSeYy4ffumacVI2VM2tkyujwh1Tc';
...
app.get('/api/geocoding/:search', async (req:Request, res:Response)=>{
    try {
        ...
        if (geocodingCache.has(search)){
            ...
            let response = await fetch(`http://www.mapquestapi.com/geocoding/v1/address?key=${GEOCODING_API_KEY}&location=${search}`)
